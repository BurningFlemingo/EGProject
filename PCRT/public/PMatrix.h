...
namespace pstd {
	...
	PSTD_API void rotate(Mat4* mat, const Rot3& rotor);
	...
	PSTD_API float calcDet(const Mat2& mat);

	PSTD_API Mat4 calcOrthoMatrix(
		float left, float right, float top, float bottom, float near, float far
	);

	PSTD_API Mat4 calcPerspectiveMatrix(
		float near, float far, float aspectRatio, float foVec
	);

	PSTD_API Mat4 calcLookAtMatrix(const Vec3& from, const Vec3& to, Vec3 up);
