...
#define INIT_FUNCTIONS(n)                                                  \
	template PSTD_API Vec<n> pstd::operator*(                              \
		const Mat<n>& mat, const Vec<n>& vec                               \
	);                                                                     \
	template PSTD_API Mat<n> pstd::operator*(                              \
		const Mat<n>& mat1, const Mat<n>& mat2                             \
	);                                                                     \
	template PSTD_API void pstd::scale(Mat<n>* mat, const Vec<n>& factor); \
	template PSTD_API void pstd::transpose(Mat<n>* mat);                   \
	template PSTD_API void pstd::setDiagonal(Mat<n>* mat, const float& val);
