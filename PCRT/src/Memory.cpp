...
Allocation pcrt::heapAlloc(size_t size) {
	...
	allocPages(
		commitSize, pcrt::ALLOC_TYPE_COMMIT, commitAddr
...
namespace {
	Freelist* createFreelist(size_t size) {
		...
		const Allocation poolAllocation{
			allocPages(allocSize, pcrt::ALLOC_TYPE_RESERVE)
		};
		allocPages(
			sizeof(Freelist), pcrt::ALLOC_TYPE_COMMIT, poolAllocation.block
		);
