...
#define INIT_FUNCTIONS(n)                                                \
	template PSTD_API Vec<n> pstd::operator+(                            \
		const Vec<n>& a, const Vec<n>& b                                 \
	);                                                                   \
	template PSTD_API Vec<n> pstd::operator-(                            \
		const Vec<n>& a, const Vec<n>& b                                 \
	);                                                                   \
	...
	template PSTD_API float pstd::dot(const Vec<n>& a, const Vec<n>& b); \
	template PSTD_API float pstd::calcMagnitude(const Vec<n>& a);        \
	template PSTD_API float pstd::calcDistance(                          \
		const Vec<n>& a, const Vec<n>& b                                 \
	);                                                                   \
	template PSTD_API void pstd::scale(Vec<n>* a, const float& factor);  \
	template PSTD_API void pstd::normalize(Vec<n>* a);                   \
	template PSTD_API Vec<n> pstd::hadamard(const Vec<n>& a, const Vec<n> b);
